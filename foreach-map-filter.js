...
function doubleValues(arr) {
    return arr.map(function (double) {
        return double * 2;
    });
}
...
function extractKey(arr, key) {
    return arr.map(function (keyArray) {
        return keyArray[key];
    });
}
...
function extractFullName(arr) {
    ...
    //     fullNameArray[i] = arr[i].first + " " + arr[i].last;
    // }
...
function removeVowels(str) {
    let stringArray = str.toLowerCase().split("");
    let vowels = "aeiou";
    return stringArray.filter(function (array) {
            return vowels.indexOf(array) === -1;
        })
        .join("");
}
